<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="个人爱好" prop="gerenaihao">
          <el-input v-model="ruleForm.gerenaihao" 
              placeholder="个人爱好" clearable  :readonly="ro.gerenaihao"></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" label="个人爱好" prop="gerenaihao">
              <el-input v-model="ruleForm.gerenaihao" 
                placeholder="个人爱好" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      </el-row>
...
export default {
  ...
  methods: {
    ...
    info(id) {
      this.$http({
        url: `gerenmingpianfenxiang/info/${id}`,
        method: "get"
      }).then(({ data }) => {
        if (data && data.code === 0) {
	...
	this.ruleForm.gerenjianjie = this.ruleForm.gerenjianjie.replace(reg,'../../../ssmhkrf8/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `gerenmingpianfenxiang/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.gerenmingpianfenxiangCrossAddOrUpdateFlag = false;
                  this.parent.search();
